...
pub fn lint_file_profiled<P: AsRef<Path>>(path: P, config: &Config) -> Result<(Vec<Diagnostic>, Option<PerfStats>)> {
    ...
    let _file_timer = if crate::logger::is_enabled(LogLevel::Perf) {
        Some(crate::logger::PerfTimer::new(format!("lint_file:{}", file_path)))
    } else {
        None
    };
...
fn build_perf_stats(timings: HashMap<String, (String, Duration)>) -> PerfStats {
    let total_time_us: u64 = timings.values()
...
pub fn lint_path_profiled<P: AsRef<Path>>(path: P, config: &Config) -> Result<(Vec<Diagnostic>, Option<PerfStats>)> {
    let path = path.as_ref();
    ...
    let overall_timer = if crate::logger::is_enabled(LogLevel::Perf) {
        Some(crate::logger::PerfTimer::new(format!("lint_path:{}", path.display())))
    } else {
        None
    };
        ...
        if is_go_file(path) {
            ...
            if let Some(s) = stats {
                for stat in s.rule_times {
                    let entry = all_timings.entry(stat.code).or_insert((stat.name, Duration::ZERO));
    ...
    let stats = if crate::logger::is_enabled(LogLevel::Perf) {
        let total_time_us: u64 = all_timings.values()
            .map(|(_, d)| d.as_micros() as u64)
            .sum();
