...
pub fn lint_massive_parallel(
    ...
    // Create scheduler
    let scheduler = HierarchicalScheduler::new(num_schedulers, mp_config.workers_per_scheduler)?;
    ...
    // Collect results
    let (diagnostics, files_count, bytes_count) = collector_handle
        .join()
