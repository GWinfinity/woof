        ...
        if node.kind() == "composite_literal" {
            ...
            // 不是字段初始化
            {
                ...
                diagnostics.push(Diagnostic {
                    code: "B001".to_string(),
                    message: "sync.Mutex 零值可直接使用，建议用 var mu sync.Mutex 替代显式初始化"
                        .to_string(),
        ...
        if node.kind() == "call_expression" {
            ...
            if normalized.starts_with("make(chan") {
                ...
                if let Some(comma_pos) = normalized.find(',') {
                    ...
                    // 提取数字 (在右括号之前)
                    let size_str: String = after_comma
                        .chars()
...
impl Rule for GoroutineLeak {
    ...
    fn check(&self, node: Node, source: &str, file_path: &str) -> Vec<Diagnostic> {
        ...
        if node.kind() == "go_statement" {
            ...
            if node_text.contains("http.ListenAndServe") || node_text.contains("signal.Notify") {
                return diagnostics;
            }
            ...
            if let Some(func) = func_scope {
                ...
                let has_done_receive =
                    func_text.contains("<-done") || func_text.contains("case <-done");
                let has_cancel =
                    func_text.contains("cancel()") || func_text.contains("defer cancel()");
                ...
                let has_sync =
                    has_wait || has_done_receive || has_cancel || has_quit || has_waitgroup_add;
    ...
    fn check(&self, node: Node, source: &str, file_path: &str) -> Vec<Diagnostic> {
        ...
        if node.kind() == "import_declaration" {
            ...
            if import_text.contains("sync/atomic") {
                ...
                diagnostics.push(Diagnostic {
                    code: "B004".to_string(),
                    message: "建议用 go.uber.org/atomic 替代 sync/atomic，提供更安全的封装"
                        .to_string(),
                    severity: self.default_severity(),
                    file_path: file_path.to_string(),
                    line: pos.row + 1,
...
fn returns_internal_field(body: Node, source: &str) -> bool {
    ...
    if let Some(ret_pos) = body_text.find("return ") {
        ...
        // 取到行尾或分号
        let ret_expr: String = after_return
            .chars()
            .take_while(|&c| c != '\n' && c != ';')
            .collect();
                ...
                if node_text.contains(open_func) {
                    ...
                    if let Some(parent) = node.parent() {
                        ...
                        if let Some(var_name) = extract_assigned_var(node, source) {
                            ...
                            if !scope_text.contains(&close_pattern)
                                && !scope_text.contains(&alt_pattern)
                            {
                                let pos = node.start_position();
                                diagnostics.push(Diagnostic {
                                    code: "B301".to_string(),
            ...
            if node_text.contains("context.WithCancel") || node_text.contains("context.WithTimeout")
            {
                ...
                if has_cancel {
                    ...
                    if let Some(parent) = node.parent() {
                        ...
                        if !scope_text.contains("defer cancel()")
                            && !scope_text.contains("cancel()")
                        {
                            let pos = node.start_position();
