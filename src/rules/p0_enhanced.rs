                ...
                if let Some(body) = node.child_by_field_name("body") {
                    ...
                    if self.has_defer_in_body(&body, source) {
                        ...
                        diagnostics.push(Diagnostic {
                            ...
                            message:
                                "range 循环中的 defer 会累积执行。考虑将逻辑提取为函数或使用闭包"
                                    .to_string(),
                            severity: self.default_severity(),
                            file_path: file_path.to_string(),
                            line: pos.row + 1,
        ...
        if node.kind() == "identifier" {
            ...
            if self.contains_non_ascii(name) {
                ...
                diagnostics.push(Diagnostic {
                    ...
                    message: format!("标识符 '{}' 包含非 ASCII 字符，建议使用英文字母", name),
                    severity: self.default_severity(),
                    file_path: file_path.to_string(),
                    line: pos.row + 1,
                    column: pos.column + 1,
        ...
        if node.kind() == "function_declaration" || node.kind() == "method_declaration" {
            if let Some(name_node) = node.child_by_field_name("name") {
                ...
                if self.contains_non_ascii(name) {
                    ...
                    diagnostics.push(Diagnostic {
                        code: "SA9002".to_string(),
                        message: format!("函数名 '{}' 包含非 ASCII 字符，建议使用英文字母", name),
                        severity: self.default_severity(),
                        file_path: file_path.to_string(),
                        line: pos.row + 1,
                        column: pos.column + 1,
...
impl Rule for StringOffsetCopy {
    fn metadata(&self) -> RuleMetadata {
        RuleMetadata {
            ...
            description:
                "从大 buffer 创建的 string 可能导致内存泄漏，建议使用 string.Clone() 或 copy",
...
impl Rule for MissingCancelCheck {
    ...
    fn check(&self, node: Node, source: &str, file_path: &str) -> Vec<Diagnostic> {
        ...
        if node.kind() == "for_statement" {
            ...
            // 检查是否是无限循环
            let is_infinite =
                text.starts_with("for {") || (text.starts_with("for ") && !text.contains(";"));
