...
use woofmt::rules::{get_enabled_rules, get_all_rules, RuleCategory, parse_version};
...
fn test_upgrade_rules_have_version() {
    ...
    assert!(!versioned_rules.is_empty(), "Should have versioned upgrade rules");
    ...
    for rule in &versioned_rules {
        ...
        assert!(meta.code.starts_with("UP"), "Versioned upgrade rules should start with UP: {}", meta.code);
    }
}
...
fn test_upgrade_rules_by_version() {
    ...
    let v22_count = versioned_rules.iter()
        .filter(|r| r.metadata().min_go_version == Some("1.22"))
        .count();
    let v23_count = versioned_rules.iter()
        .filter(|r| r.metadata().min_go_version == Some("1.23"))
        .count();
    let v24_count = versioned_rules.iter()
        .filter(|r| r.metadata().min_go_version == Some("1.24"))
        .count();
    let v25_count = versioned_rules.iter()
...
fn count_enabled_versioned_upgrade_rules(config: &Config) -> usize {
    get_enabled_rules(config).into_iter()
        .filter(|r| {
            let meta = r.metadata();
            meta.category == RuleCategory::Upgrade && meta.min_go_version.is_some()
        })
...
fn test_version_filtering_go123() {
    ...
    let count = count_enabled_versioned_upgrade_rules(&config);
    assert_eq!(count, 10, "Go 1.23 should have 10 versioned upgrade rules (5 + 5)");
}
...
fn test_version_filtering_go124() {
    ...
    let count = count_enabled_versioned_upgrade_rules(&config);
    assert_eq!(count, 14, "Go 1.24 should have 14 versioned upgrade rules (5 + 5 + 4)");
}
...
fn test_version_filtering_go125() {
    ...
    let count = count_enabled_versioned_upgrade_rules(&config);
    assert_eq!(count, 18, "Go 1.25 should have all 18 versioned upgrade rules");
}
...
fn test_other_upgrade_rules_always_enabled() {
    ...
    let always_enabled_upgrades: Vec<_> = all_enabled.iter()
        .filter(|r| {
            let meta = r.metadata();
    ...
    assert!(!always_enabled_upgrades.is_empty(), "Should have always-enabled upgrade rules (GEN, WS, etc.)");
...
fn test_rules_list_includes_upgrade() {
    ...
    let all_rules = get_all_rules();
    let upgrade_codes: Vec<_> = all_rules.iter()
